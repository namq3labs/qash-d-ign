...
export function RecurringTransferModal({ isOpen, onClose, zIndex }: ModalProp<RecurringTransferModalProps>) {
  ...
  //       amount: payment.amount,
  //       metadata: {
  //         symbol: payment.tokens[0]?.metadata?.symbol || "USDT",
  ...
  //       amountLabel: `${tx.assets[0]?.amount} ${tx.assets[0]?.metadata?.symbol || "USDT"}`,
  ...
  //       return {
  //         amountLabel: `${payment.amount} ${payment.tokens[0]?.metadata?.symbol || "USDT"}`,
