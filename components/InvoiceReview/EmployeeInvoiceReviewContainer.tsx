...
// Stubbed: @getpara/react-sdk-lite and @miden-sdk/use-miden-para-react removed
const useParaModal = () => ({ open: () => {}, close: () => {} });
const useWallet = () => ({ wallets: [], selectedWallet: null });
const useParaMiden = (..._args: any[]) => ({ para: null, evmWallets: [] });
const useParaAccount = () => ({ isConnected: false, embedded: { wallets: [] } });
  ...
  const mapApiResponseToInvoiceData = (apiData: any): InvoiceData => {
    ...
    return {
      ...
      from: {
        ...
        token: n(fromDetails.token?.symbol || apiData.payroll?.token?.symbol),
