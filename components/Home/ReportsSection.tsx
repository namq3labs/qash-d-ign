...
const TAX_JURISDICTIONS: TaxJurisdiction[] = [
  { code: "SG", name: "Singapore", flag: "\uD83C\uDDF8\uD83C\uDDEC", corpTaxRate: 17, cryptoSwapTaxable: false, payrollTaxable: true, filingDeadline: "Nov 30 (paper) / Dec 15 (e-filing)", notes: "No capital gains tax. Crypto trading income taxed at 17% if it is your business activity. Payment tokens exempt from GST." },
  { code: "HK", name: "Hong Kong", flag: "\uD83C\uDDED\uD83C\uDDF0", corpTaxRate: 16.5, cryptoSwapTaxable: false, payrollTaxable: true, filingDeadline: "Apr 1 (employer returns)", notes: "8.25% on first HK$2M profit, 16.5% above. No capital gains tax. Territorial source principle, offshore profits not taxed." },
  { code: "US", name: "United States", flag: "\uD83C\uDDFA\uD83C\uDDF8", corpTaxRate: 21, cryptoSwapTaxable: true, payrollTaxable: true, filingDeadline: "Apr 15 (C-corp) / Mar 15 (S-corp)", notes: "Every crypto-to-crypto swap is a taxable event. Stablecoin payroll subject to FICA withholding. Most aggressive enforcement globally." },
  { code: "AE", name: "UAE", flag: "\uD83C\uDDE6\uD83C\uDDEA", corpTaxRate: 9, cryptoSwapTaxable: false, payrollTaxable: false, filingDeadline: "9 months after FY end", notes: "9% on profits above AED 375K. Free zone companies (DMCC, DIFC, ADGM) can qualify for 0%. No personal income tax." },
  { code: "CH", name: "Switzerland", flag: "\uD83C\uDDE8\uD83C\uDDED", corpTaxRate: 14.6, cryptoSwapTaxable: true, payrollTaxable: true, filingDeadline: "Mar-Sep (varies by canton)", notes: "Combined rate 11.9-21.6% depending on canton. Zug (Crypto Valley) ~11.9%. All crypto gains are business income for companies." },
  { code: "KY", name: "Cayman Islands", flag: "\uD83C\uDDF0\uD83C\uDDFE", corpTaxRate: 0, cryptoSwapTaxable: false, payrollTaxable: false, filingDeadline: "No tax filing required", notes: "Zero income tax, capital gains tax, and withholding tax. Economic substance requirements apply. CIMA registration for VASPs." },
];
...
function getStatusBadge(status: string): BadgeStatus {
  switch (status) {
    ...
    case "PENDING": return BadgeStatus.AWAITING;
    case "FAILED": return BadgeStatus.FAIL;
    default: return BadgeStatus.NEUTRAL;
  }
}
...
const PieChartSection = ({ data, emptyMessage }: { data: { name: string; value: number; color: string }[]; emptyMessage: string }) => {
  if (data.length === 0) {
...
export const ReportsSection = () => {
  ...
  const jurisdiction = useMemo(() => {
    const match = TAX_JURISDICTIONS.find((j) =>
      companyCountry.toLowerCase().includes(j.name.toLowerCase()) ||
      companyCountry.toUpperCase() === j.code
    );
  ...
  const buildCSVContent = (txs: DemoTransaction[]): string => {
    const headers = [
      "Date", "Timestamp (UTC)", "Transaction ID", "Transaction Hash", "Explorer Link",
      "Type", "Direction", "From (Name)", "From (Address)", "To (Name)", "To (Address)",
      "Account ID", "Amount", "Currency", "Category", "Description", "Status", "Notes",
    ];
    const rows = txs.map((tx) => {
      const isInflow = tx.type === "RECEIVE" || tx.type === "INVOICE";
      return [
        formatDate(tx.timestamp), tx.timestamp, tx.id, tx.txHash,
        `${MIDEN_EXPLORER_URL}/tx/${tx.txHash}`, tx.type, isInflow ? "Inflow" : "Outflow",
        tx.from, tx.fromAddress, tx.to, tx.toAddress, tx.accountId,
        isInflow ? tx.amount.toString() : `-${tx.amount}`, tx.currency, tx.category,
        tx.label || "", tx.status, tx.note || "",
      ].map(escapeCSV);
    });
    ...
    <div className="flex flex-col w-full gap-6 px-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <SecondaryButton text="Export CSV" variant="light" onClick={() => handleExport("CSV")} buttonClassName="w-auto px-4" />
          <SecondaryButton text="Export PDF" variant="light" onClick={() => handleExport("PDF")} buttonClassName="w-auto px-4" />
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card
              ...
              amount={formatCurrency(monthlyFinancials.length > 0 ? Math.round(totalIncome / monthlyFinancials.length) : 0)}
              info="Average monthly income across the period"
            />
            <Card title="Income Transactions" amount={filteredTransactions.length.toString()} info="Number of income transactions" />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard title="Monthly Income">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyFinancials.map((m) => ({ month: m.month, Income: m.income }))} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card
              ...
              amount={formatCurrency(monthlyFinancials.length > 0 ? Math.round(totalExpenses / monthlyFinancials.length) : 0)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard title="Monthly Expenses">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyFinancials.map((m) => ({ month: m.month, Expenses: m.expenses }))} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--primary-divider)" />
                  <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                  <Tooltip content={<ChartTooltip />} />
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card
              ...
              amount={formatCurrency(monthlyFinancials.length > 0 ? Math.round(totalPayroll / monthlyFinancials.length) : 0)}
              info="Average monthly payroll across the period"
            />
            <Card title="Treasury Balance" amount={formatCurrency(treasuryBalance)} info="Current total balance across all accounts" />
            <Card
              title="Runway"
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyFinancials.map((m) => ({ month: m.month, Payroll: m.payroll }))} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--primary-divider)" />
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                <Tooltip content={<ChartTooltip />} />
      ...
      {activeTab === "tax" && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card title={`Est. Tax (${jurisdiction.corpTaxRate}%)`} amount={formatCurrency(estimatedTax > 0 ? estimatedTax : 0)} info={`Estimated at ${jurisdiction.name} corporate tax rate`} />
            <Card title="Filing Deadline" amount={jurisdiction.filingDeadline} info={`Tax filing deadline for ${jurisdiction.name}`} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <ChartCard title="Income vs Deductible Expenses">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyFinancials.map((m) => ({ month: m.month, Income: m.income, Expenses: m.expenses }))} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--primary-divider)" />
                  <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "var(--text-secondary)", fontSize: 12 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                  <Tooltip content={<ChartTooltip />} />
            ...
            <div className="bg-background border border-primary-divider rounded-2xl p-5 flex flex-col gap-5">
              <div>
                <h3 className="text-base font-medium text-text-primary mb-4">
                  {jurisdiction.flag} Tax Breakdown ({jurisdiction.name})
                </h3>
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-text-secondary">Taxable Income</span>
                    <span className="text-base font-semibold text-text-primary">{formatCurrency(netFlow > 0 ? netFlow : 0)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-base font-semibold text-badge-fail-text">{formatCurrency(estimatedTax > 0 ? estimatedTax : 0)}</span>
                  </div>
              ...
              <div className="border-t border-primary-divider pt-4 flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-semibold ${jurisdiction.cryptoSwapTaxable ? "text-badge-fail-text" : "text-badge-success-text"}`}>
                    {jurisdiction.cryptoSwapTaxable ? "Yes" : "No"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-text-secondary">Payroll in stablecoins taxable</span>
                  <span className={`text-sm font-semibold ${jurisdiction.payrollTaxable ? "text-badge-awaiting-text" : "text-badge-success-text"}`}>
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-base font-medium text-text-primary">
            {activeTab === "income" ? "Income Transactions"
              ...
              : activeTab === "payroll" ? "Payroll Transactions"
              : "All Transactions"}
          </h3>
