    ...
    return allLinks.map((link: PaymentLinkType) => ({
      ...
      Amount: (
        <div className="flex justify-center items-center gap-1">
          ...
          <img
            src={`/token/${(link.acceptedTokens?.[0]?.symbol || "any-token").toLowerCase()}.svg`}
            onError={(e) => { (e.target as HTMLImageElement).src = blo(turnBechToHex(link.acceptedTokens?.[0]?.address || "")); }}
