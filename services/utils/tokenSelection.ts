...
export const getDefaultSelectedToken = (assets: AssetWithMetadata[]): AssetWithMetadata => {
  // If user has no assets, default to USDT
  if (assets.length === 0) {
    return {
      amount: "0",
  ...
  // Find the token with the highest balance (excluding USDT if it has 0 balance)
  const nonZeroAssets = assets.filter(asset => parseFloat(asset.amount) > 0);

  if (nonZeroAssets.length === 0) {
    // All assets have 0 balance, default to USDT
    return {
      faucetId: QASH_TOKEN_ADDRESS,
      amount: "0",
      metadata: {
