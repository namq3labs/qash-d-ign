...
export async function getBalance(client: import("@miden-sdk/miden-sdk").WebClient, address: string) {
  ...
  const assetsWithMetadata = await Promise.allSettled(
    assets.map(async asset => {
      ...
      try {
        ...
        // If this is the USDT token, use known constants instead of UNKNOWN
        ...
        return {
          assetId: faucetId,
          balance: (Number(asset.amount()) / Math.pow(10, 6)).toString(), // Default to 6 decimals
          decimals: 6,
          maxSupply: "0",
          symbol: "UNKNOWN",
        };
  ...
  // Ensure USDT token is always present
  const qashTokenIndex = balances.findIndex(asset => asset.assetId === QASH_TOKEN_ADDRESS);

  if (qashTokenIndex >= 0) {
    // USDT token exists, always ensure it has correct metadata (even if it was set to "UNKNOWN" due to fetch failure)
    ...
    // USDT token doesn't exist, add it with balance 0
    balances.push({
