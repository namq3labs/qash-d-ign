...
export interface DemoEmployee {
  ...
  walletAddress: string;
  email: string;
  /** Primary group (kept for back-compat with payroll/bill displays). */
  groupId: number;
  /** All groups this employee belongs to. An employee can be in multiple groups. */
  groupIds?: number[];
  companyId: number;
...
export function DemoProvider({ children }: { children: ReactNode }) {
  ...
  // Memoized so the context value is stable across re-renders (e.g. navigation,
  // when none of the underlying state changed). All callbacks are useCallback-stable,
  // so this only recomputes when the data/auth state actually changes — preventing
  // every useDemo() consumer from re-rendering on each route change.
  const value: DemoContextType = React.useMemo(
    () => ({
      data,
      isLoaded,
      isLoggedIn,
